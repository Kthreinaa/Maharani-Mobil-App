...
@php
  ...
  $pageTitle = 'Pantau Pesanan';
@endphp

@section('content')
  <div class="mb-4 rounded-[1.4rem] border border-slate-200 bg-white/75 px-5 py-4 text-sm text-slate-500 shadow-sm">
    Marketing hanya memantau progres pesanan customer dari halaman ini. Seluruh pengelolaan order dilakukan oleh supervisor.
  </div>

  <div class="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
    <form class="flex flex-wrap gap-2" method="GET" action="{{ route('marketing.orders.index') }}">
        ...
        @forelse($orders as $order)
          ...
          <tr class="hover:bg-white/70">
            ...
            <td class="px-5 py-4 text-right">
              <a class="rounded-full border border-slate-200 bg-white/80 px-4 py-2 text-xs font-semibold text-slate-700" href="{{ route('marketing.orders.show', $order) }}">Detail</a>
            </td>
          </tr>
        @empty
