    ...
    <div class="rounded-[2rem] border border-white/70 bg-[rgba(255,255,255,0.72)] p-5 shadow-[0_24px_70px_rgba(15,23,42,0.10)] backdrop-blur-[24px]">
      <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Cari berdasarkan nama customer, merk mobil, tipe, tahun, atau kode unit. Marketing tetap bisa memantau siapa yang mengelola penawaran tanpa membingungkan tim.</p>
        </div>
      ...
      <form class="mt-5 flex flex-col gap-3 xl:flex-row xl:flex-wrap xl:items-center" method="GET" action="{{ route('marketing.offers.index') }}">
        <input
        ...
        <a class="rounded-full border border-slate-200 bg-white/86 px-5 py-3 text-sm font-semibold text-slate-600" href="{{ route('marketing.offers.index') }}">Reset</a>
      </form>
          ...
          @if ($offer->handled_at)
            <div class="mt-4 rounded-[1.4rem] border border-sky-200/80 bg-[linear-gradient(135deg,rgba(239,246,255,0.95),rgba(255,255,255,0.92))] p-4 shadow-sm">
              <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div class="min-w-0">
                  <p class="text-xs font-bold uppercase tracking-[0.18em] text-sky-600">Tindak Lanjut Internal</p>
                  <p class="mt-2 text-lg font-extrabold text-slate-900">{{ $offer->status_label }}</p>
                  <p class="mt-1 text-sm text-slate-500">
                    Ditangani oleh {{ $offer->handled_role === 'supervisor' ? ($offer->handledBy?->name ?? 'Supervisor') : 'Supervisor' }} pada {{ $offer->handled_at->translatedFormat('d M Y H:i') }}.
                  </p>
                </div>
