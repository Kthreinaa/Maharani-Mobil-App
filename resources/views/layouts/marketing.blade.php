...
<head>
  ...
  <title>{{ $title ?? 'Marketing Dashboard' }}</title>
...
@php
  $navItems = [
    ['label' => 'Dashboard', 'route' => 'marketing.dashboard', 'match' => 'marketing.dashboard', 'icon' => 'dashboard'],
    ['label' => 'Pantau Unit Mobil', 'route' => 'marketing.products.index', 'match' => 'marketing.products.*', 'icon' => 'directions_car'],
    ['label' => 'Aktivitas Customer', 'route' => 'marketing.customers.index', 'match' => 'marketing.customers.*', 'icon' => 'groups'],
    ['label' => 'Pantau Penawaran', 'route' => 'marketing.offers.index', 'match' => 'marketing.offers.*', 'icon' => 'sell'],
    ['label' => 'Pantau Pesanan', 'route' => 'marketing.orders.index', 'match' => 'marketing.orders.*', 'icon' => 'receipt_long'],
    ['label' => 'Pantau Transaksi', 'route' => 'marketing.transactions.index', 'match' => 'marketing.transactions.*', 'icon' => 'payments'],
  ];
@endphp
      ...
      <aside class="hidden w-[298px] shrink-0 lg:block">
        <div class="flex h-full flex-col overflow-visible rounded-[2rem] border border-white/70 bg-[rgba(255,255,255,0.72)] p-5 shadow-[0_24px_70px_rgba(15,23,42,0.12)] backdrop-blur-[26px]">
          <div class="rounded-[1.6rem] bg-[linear-gradient(135deg,#08132e_0%,#102a63_56%,#f5a623_140%)] px-5 py-5 text-white shadow-[0_18px_40px_rgba(8,19,46,0.24)]">
            <p class="text-[10px] font-semibold uppercase tracking-[0.24em] text-[#f7c35f]">Marketing Workspace</p>
            <div class="mt-3 text-[28px] font-extrabold tracking-tight">Maharani Mobil</div>
            <p class="mt-2 text-xs text-slate-100/90"></p>
          </div>
          ...
          <div class="mt-auto rounded-[1.4rem] border border-slate-200/80 bg-white/70 p-4">
            ...
            <p class="mt-2 text-sm font-semibold text-slate-900">{{ auth()->user()->name ?? 'Marketing' }}</p>
            <p class="mt-1 text-xs leading-5 text-slate-500">Akses marketing aktif untuk pantau unit, customer, dan follow-up awal calon pembeli.</p>
          ...
          <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="min-w-0">
              <div class="flex items-center gap-3 lg:hidden">
                <span class="material-symbols-outlined rounded-full bg-[#08132e] p-2 text-white">campaign</span>
                <span class="font-headline text-lg font-extrabold text-slate-900">Maharani Mobil</span>
              </div>
              <h1 class="mt-1 font-headline text-[24px] font-extrabold tracking-tight text-slate-900 md:text-[28px]">{{ $pageTitle ?? 'Marketing' }}</h1>
              <p class="mt-1 text-sm text-slate-500">Marketing Control Panel</p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
              ...
              @isset($dashboardNotifications)
                <x-dashboard-notification-bell :data="$dashboardNotifications" title="Notifikasi Marketing" />
              @endisset
              <a class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/80 px-4 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:bg-white" href="{{ route('marketing.settings.edit') }}">
              ...
              <div class="inline-flex h-11 min-w-11 items-center justify-center rounded-full bg-[linear-gradient(135deg,#08132e_0%,#102a63_100%)] px-3 text-sm font-bold text-white shadow-[0_16px_30px_rgba(8,19,46,0.18)]">
                {{ strtoupper(substr(auth()->user()->name ?? 'MK', 0, 2)) }}
              </div>
            </div>
          </div>
        </div>
