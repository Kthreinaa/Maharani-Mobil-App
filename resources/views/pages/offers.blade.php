...
<head>
  ...
  <meta name="description" content="Ajukan penawaran harga untuk unit pilihan Anda."/>
...
<body class="bg-background text-on-background min-h-screen flex flex-col">
  ...
  <main class="max-w-screen-2xl mx-auto w-full px-6 md:px-12 py-10 flex-grow">
    <h1 class="text-3xl font-extrabold text-primary mb-6">Ajukan Penawaran</h1>
    <!-- API: POST /api/offers -->
    <section class="bg-white rounded-2xl shadow-xl shadow-blue-900/5 p-8 max-w-2xl">
      <div class="space-y-6">
        <div>
          <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">Unit</label>
          <input class="w-full rounded-xl border border-outline-variant p-4" value="Toyota Camry 2.5 V Hybrid 2022" type="text"/>
        </div>
        <div>
          <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">Harga Penawaran</label>
          <input class="w-full rounded-xl border border-outline-variant p-4" placeholder="Rp 520.000.000" type="text"/>
        </div>
        <div>
          <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">Catatan</label>
          <textarea class="w-full rounded-xl border border-outline-variant p-4 h-24" placeholder="Alasan penawaran atau detail tambahan"></textarea>
        </div>
        <button class="w-full bg-secondary-container text-on-secondary-fixed py-4 rounded-xl font-bold">Kirim Penawaran</button>
      </div>
    </section>
  </main>
